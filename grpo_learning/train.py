...
class GSM8KDataset(Dataset):
    def __init__(self, data_path, tokenizer, num_samples=None, seed=42):
        
        self.tokenizer = tokenizer
        data = load_dataset(data_path)
        self.data = data['train']
        if num_samples is not None and num_samples < len(self.data):
            self.data = self.data.shuffle(seed=seed).select(range(num_samples))
    ...
    def train_step(self, model, inputs, optimizer, step):
        ...
        if (step + 1) % self.args.gradient_accumulation_steps == 0:
            ...
            if self.writer is not None:
                self.writer.add_scalar("grpo_loss", loss.item(), self.update_steps)
...
"""
    ...
    writer = SummaryWriter('/root/autodl-fs')
    # 策略模型
    tokenizer = AutoTokenizer.from_pretrained('/root/autodl-fs/Qwen3-0.6B')
    model = AutoModelForCausalLM.from_pretrained('/root/autodl-fs/Qwen3-0.6B')
    # 奖励函数
    # reward_model = '/root/autodl-fs/reward-model-deberta-v3-large-v2'
    # reward_tokenizer = AutoTokenizer.from_pretrained('/root/autodl-fs/reward-model-deberta-v3-large-v2')


    
    
    prompts_dataset = GSM8KDataset('/root/autodl-fs/data_gsm8k/main', tokenizer, num_samples=100)
